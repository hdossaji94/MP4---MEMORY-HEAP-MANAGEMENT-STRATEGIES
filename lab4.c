...
int main(int argc, char **argv)
{
    ...
    else if (dprms.UnitDriver == 1)
    {
         ...
         */
        printf("The size of chunk_t is %lu bytes\n", sizeof(chunk_t)); 

           /* How many units are in the free list if you start with one page?
        ...
        // units_in_first_page might be 512, 511, 256, or 255 
        // depending on your implementation
        int units_in_first_page = 4096/unit_size;
        assert(units_in_first_page * unit_size == 4096);
        // do you need to subtract one unit for the dummy?  If so,
        ...
        // then request  511 - (62+1) - (254+1) -1 = 192 units
        ...
        printf("\n----- End unit test driver 1 -----\n");
            ...
            printf("need 255 units or the entire block"); // This special first case sees if it the entire block will be removed as soon as it is initialized. 
            Mem_print();
            Mem_free(z);
            printf("should return back the entire block");
            Mem_print();
            struct chunk_t *q = Mem_alloc(sizeof(chunk_t)*4); // Once the block that was removed returns back to the free list, mem_alloc calls will begin shaving from it. 
            printf("need 4 units\n");
            Mem_print();
            ...
            Mem_print();
            Mem_free(a); // When coalescing is on, these memory blocks will join with the next and previous 
            printf("should return back 112\n");
            Mem_print();  
            struct chunk_t *y = Mem_alloc(sizeof(chunk_t)*1);
            printf("need 1 units\n");
            Mem_print();
            Mem_free(q);   // There is a variety of different mem_allocs, mem_frees and mem_prints in this unit driver.
            Mem_free(w);    // This ending allows all the memory that was malloced to be returned back to the free list
            Mem_free(e);    // just as a real program and a user would do. finally mem_stats will report if all memory
            Mem_free(r);    // has been returned and some other information
            Mem_free(t);
            Mem_free(y);    // in the end all the memory blocks are returned and linked or if coalescing is on then all
...
void equilibriumDriver(driver_params *ep)
{
    ...
    for (i = 0; i < ep->WarmUp; i++) {
        ...
        list_insert(mem_list, (data_t *) ptr, list_iter_first(mem_list));
        ptr = NULL;
    }
    printf("After warmup\n");
    if (!ep->SysMalloc) {
    ...
    for (i = 0; i < ep->Trials; i++) {
        if (drand48() < 0.5) {
            ...
            idx_ptr = list_iter_first(mem_list);
    ...
    for (i = 0; i < pos; i++) {
        ptr = (int *) list_remove(mem_list, list_iter_first(mem_list));
        assert(ptr != NULL);
        size = -ptr[0];
        assert(min_num_ints <= size && size <= ep->AvgNumInts+ep->RangeInts);
        for (index = 1; index < size; index++)
